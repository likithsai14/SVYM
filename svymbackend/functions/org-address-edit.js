...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!oldAddress || !newAddress || typeof oldAddress !== 'string' || typeof newAddress !== 'string' || oldAddress.trim() === '' || newAddress.trim() === '') {
      return {
        statusCode: 400,
        body: JSON.stringify({ message: 'Both oldAddress and newAddress are required and must be non-empty strings' })
      };
    }
    ...
    if (org.contactus.addresses.includes(newAddress.trim()) && newAddress.trim() !== oldAddress.trim()) {
      return {
        statusCode: 400,
