document.addEventListener("DOMContentLoaded", function () {
  ...
  const formTrainerSecurityQuestion = document.getElementById(
    "formTrainerSecurityQuestion"
  );
  const formTrainerSecurityAnswer = document.getElementById(
    "formTrainerSecurityAnswer"
  );
  ...
  function openAddEditTrainerModal(userId = null) {
    ...
    if (userId) {
      ...
      if (trainerToEdit) {
        ...
        formTrainerSecurityQuestion.value =
          trainerToEdit.securityQuestion || "";
    ...
    trainerForm.addEventListener("submit", async function (event) {
      ...
      if (!formTrainerContact.value && !formTrainerEmail.value) {
        showFormMessage(
          formTrainerMessage,
          "error",
          "Please provide at least one contact detail: Email or Phone."
        );
      ...
      if (!isEditMode) {
        ...
        if (response.ok) {
          ...
          showFormMessage(
            formTrainerMessage,
            "error",
            `Failed to create the trainer. ${trainer.message || ""}`
          );
        }
      } else {
        ...
        const trainer = response.json();
        if (response.ok) {
          if (response.status == 200) {
            showFormMessage(
              formTrainerMessage,
              "success",
              `Trainer ${trainer.id} updated successfully!`
            );
          }
        } else {
          showFormMessage(
            formTrainerMessage,
            "error",
            "Error: Trainer not found for update."
          );
